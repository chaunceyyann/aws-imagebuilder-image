...
def _import_logger_utils():
    """Import logger_utils module dynamically."""
    utils_path = os.path.join(os.path.dirname(__file__), "..", "utils")
    spec = importlib.util.spec_from_file_location(
        "logger_utils", os.path.join(utils_path, "logger_utils.py")
    )
...
# Import logger_utils dynamically
logger_utils = _import_logger_utils()


def lambda_handler(event, context):
    logger = logger_utils.get_logger("ami_cleanup")
    ...
    response = ec2_client.describe_images(
        Filters=[{"Name": f"tag:{tag_key}", "Values": [tag_value]}], Owners=["self"]
    )
    ...
    for ami in amis[keep_latest:]:
        ...
        for device in ami.get("BlockDeviceMappings", []):
            if "Ebs" in device and "SnapshotId" in device["Ebs"]:
                ...
                logger.info(f"Deleting snapshot {snapshot_id} for AMI {ami_id}")
